...
func Test_MasterCommand_listTags(t *testing.T) {
	...
	t.Run("listTags_repo_error", func(t *testing.T) {
		...
		require.Nil(t, err)
		gotErr := h.MasterCommand(msg)
		require.Equal(t, fmt.Errorf("list tags: %w", fmt.Errorf("repository.ListTags: %w", repoErr)), gotErr)
	})
...
func Test_MasterCommand_subscribe(t *testing.T) {
	...
	t.Run("private_channel_success", func(t *testing.T) {
		...
		client.EXPECT().SubscribeToChannel(subID).Return(bot.PrivateChannel)
